...
@section('script')
    <script>
        ...
        function Remove() {
            let id = $('#id').val();
            let url = '{{ route('admin.menu.destroy',['menu'=>':menu']) }}';
        ...
        function GroupRemove() {
            let id = $('#id').val();
            let url = '{{ route('admin.menu.GroupRemove') }}';
            $.ajax({
                url: url,
                data: {
                    _token: "{{ csrf_token() }}",
...
@section('content')
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex justify-content-between">
            <h1 class="w-50">منو های صفحه اصلی</h1>
            <div class="d-flex w-50">
                <select onchange="show_per_page(this,'{{ route('admin.menus.index') }}')" class="form-control form-control-sm ms-3">
                ...
                <a title="اضافه کردن منو جدید" href="{{ route('admin.menu.create') }}"
                                ...
                                <div class="col-12">
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        @foreach($menus as $key => $item)
                                            ...
                                            <tr>
                                                <th scope="col">{{ $menus->firstItem()+$key }}</th>
                                                <th scope="col">{{ $item->title_fa ?? $item->title_en }}</th>
                                                <th scope="col">{{\Modules\Admin\Entities\Menu::parentTitle($item->id)}}</th>
                                                <th scope="col">
