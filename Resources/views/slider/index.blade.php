...
@section('script')
    <script>
        ...
        function Remove() {
            let id = $('#id').val();
            let url = '{{ route('admin.slider.destroy',['slider'=>':slider']) }}';
            url = url.replace(':slider', id);
            $.ajax({
                url: url,
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        function GroupRemove() {
            let id = $('#id').val();
            let url = '{{ route('admin.slider.GroupRemove') }}';
            $.ajax({
                url: url,
                data: {
                    _token: "{{ csrf_token() }}",
...
@section('content')
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex justify-content-between">
            <h1 class="w-50">اسلایدر های صفحه اصلی</h1>
            <div class="d-flex w-50">
                <select onchange="show_per_page(this,'{{ route('admin.sliders.index') }}')" class="form-control form-control-sm ms-3">
                ...
                <a title="اضافه کردن اسلایدر جدید" href="{{ route('admin.slider.create') }}"
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col">
                                                    ...
                                                    <a title="EDIT" class="btn btn-sm btn-info mr-3"
                                                       href="{{ route('admin.slider.edit' , ['slider' => $item->id]) }}">
