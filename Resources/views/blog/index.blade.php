...
@section('script')
    <script>
        ...
        function Remove() {
            let id = $('#id').val();
            let url = '{{ route('admin.blog.destroy',['blog'=>':blog']) }}';
        ...
        function GroupRemove() {
            let id = $('#id').val();
            let url = '{{ route('admin.blog.GroupRemove') }}';
            $.ajax({
                url: url,
                data: {
                    _token: "{{ csrf_token() }}",
...
@section('content')
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex justify-content-between">
            <h1 class="w-50">مقاله های صفحه اصلی</h1>
            <div class="d-flex w-50">
                <select onchange="show_per_page(this,'{{ route('admin.blogs.index') }}')" class="form-control form-control-sm ms-3">
                ...
                <a title="اضافه کردن مقاله جدید" href="{{ route('admin.blog.create') }}"
                                ...
                                <div class="col-12">
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        @foreach($blogs as $key => $item)
                                            ...
                                            <tr>
                                                <th scope="col">{{ $blogs->firstItem()+$key }}</th>
                                                ...
                                                <th scope="col">
                                                    <img class="img-thumbnail image_in_admin_table"
                                                         src="{{ imageExist(env('BLOG_IMAGE_FILE'),$item->image) }}">
                                                </th>
                                                <th scope="col">
                                                    ...
                                                    <a title="EDIT" class="btn btn-sm btn-info mr-3"
                                                       href="{{ route('admin.blog.edit' , ['blog' => $item->id]) }}">
